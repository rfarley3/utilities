...
# Nov 2014 rfarley3
#
# Outputs marked up pdf of diff between 2 most recent versions of a latex document
# Can handle a document that consists of many tex files
#
# Assumes
#     * '\n' line endings
#     * you only have 1 file with documentclass (call the mainfile)
#          * this is the file you would feed into latex to compile, e.g. pdflatex <mainfile>
#     * you are calling from the dir that contains the mainfile
#     * that the bbl file in the current directory is good enough (so your refs may be wrong)
...
#   as well as its symlinking method with the current files was not working as expected
#
# TODO: 
...
def main ():
	...
	mainbase = mainfile[:-4]
	# TODO doesn't handle multiple results, just assumes first match is the correct answer
	...
	newest_commit = str (_cmd_l("git log --pretty=oneline --abbrev=commit -n 1") ).split(" ")[0]
	#print (newest_commit)
	...
	if wc_l == 0: # a repo without changes has 0 lines in diff
		...
		old = str (_cmd_l("git log --pretty=oneline --abbrev=commit -n 2 | tail -1") ).split(" ")[0]
		out = "diff-" + timestamp + "-" + old[0:7] + "-" + new[0:7] + ".pdf"

	# you can use the above as a wrapper to auto call latexdiff (should it work for you)
	# to do that, use this command string:
	#     cmd = "git latexdiff --ignore-latex-errors --latexmk --output " + out + " " + old + " " + new
	...
	print ("Dircall_rel " + dircall_rel) # path between base of git repo and calling directory
	...
	# use git archive to pull a tarball of the version you want into the temp dir
	print ("Getting old version")
	os.chdir(dirtmp)
	_cmd ("(cd " + dirgit + " && git archive --format=tar " + old + " " + dircall + ") | tar -xf -")
	os.chdir(dircall_rel)
	print ("Curr dir: " + _cmd_l("pwd") )

	# we assume that the bbls aren't tracked
	print ("Making bbl for old")
	# we assume that the current bbl is good enough for the old one so need to make it
	...
	# TODO git archive the new if new isn't -- so you can do arbitrary commit diffs
	# for now, you aren't diffing anything other than the latest two
	# if most recent commit != current, then you are doing: diff     <most recent commit> <dircall>
	# if most recent commit == current, then you are doing: diff <2nd most recent commit> <dircall>
	print ("Latexpanding new")
	_cmd_p ("(cd " + dircall + " && latexpand \"" + mainbase + "\".tex) > new-flattened.tex")

	# this does the heavy lifting, converts two tex files into a word diff that is a valid tex
	...
	if repair_graphics:
		...
		# this assumes you're me... that you have all your figs not in the tex dir
		# more precisely that they are in the parent dir under Figs
		# e.g. that you have a ../Figs/<fig> convention
		# this searches for relative Figs path and makes it absolute so that the tex can compile
		_cmd_p ("LC_CTYPE=C sed 's/\.\.\/Figs/" + dirgit + "\/Figs/' diff.tex > \"" + mainfile + "\"")
	...
	# now copy the result from the temporary directory to the directory you called this script from
	print ("publishing")
	pdfout = dircall + "/" + out
	...
	print ("")
	# I use skim, here is a copy and paste to launch skim on that new pdf you just made
